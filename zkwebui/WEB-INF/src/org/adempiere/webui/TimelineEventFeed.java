...
public class TimelineEventFeed extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		MAssignmentSlot[] slots = getAssignmentSlots(resourceId, startDate, endDate);
		...
		for (MAssignmentSlot slot : slots) {
			xml.append("<event ").append("\r\n");
			xml.append(XmlFns.attr(
					"start",
					TimelineUtil.formatDateTime(
							new Date(slot.getStartTime().getTime()))));
			if (slot.getEndTime() != null) {
				xml.append("\r\n");
				xml.append(XmlFns.attr(
						"end",
						TimelineUtil.formatDateTime(
								new Date(slot.getEndTime().getTime()))));
				xml.append("\r\n");
				xml.append(XmlFns.attr("isDuration", "true"));
			}
			xml.append(XmlFns.attr(
					"color",
					"#" + ZkCssHelper.createHexColorString(slot.getColor(true))));
			xml.append("\r\n")
					.append(XmlFns.attr("title", slot.getName()))
					.append("\r\n")
					.append(">");
			if (slot.getDescription() != null
					&& slot.getDescription().trim().length() > 0) {
				xml.append("\r\n")
			...
			if (slot.getMAssignment() != null) {
				String link = "<a href=\"javascript:void(0)\" onclick=\""
						...
						+ "zkau.send({uuid: '" + uuid
						+ "', cmd: 'onClick', data: "
						+ "[" + slot.getMAssignment().getS_ResourceAssignment_ID()
						+ ", 0], ctl: true})\">Edit</a>";
	...
	protected void prepareContext(HttpServletRequest req) {
	...
	protected MAssignmentSlot[] getAssignmentSlots(
		...
		ScheduleUtil model = new ScheduleUtil(Env.getCtx());
		return model.getAssignmentSlots(
				resourceId, startDate, endDate, null, true, null);
	}
}
