...
>
> **Status:** Phases 1-4 and Phase 5a-5f are complete. Phase 5g-0, the
> Business Partner oracle and its amendments, and Phase 5g-1b modern CRUD parity
> are accepted on `develop`; the first portable modern-business demo is accepted
> from workflow run
> [33994400756](https://github.com/samqbush/adempiere2/actions/runs/33994400756).
> Phase 5g-1c through 5g-7, Phase 5h, Phase 6, and Phase 7 remain open.
> Required-check enforcement remains repository-administrator action R8. The
> measured completion and remaining-effort forecast is maintained in
> [`docs/modernization/modernization-completion-forecast.md`](docs/modernization/modernization-completion-forecast.md).
...
| R14 | Workflow attribution oracle | The frozen `5g-1a-x` answer recorded `AD_WF_Process` and `AD_WF_Activity` with startup-context attribution and left `AD_WF_EventAudit` ambient. The accepted domain decision is that a document-triggered workflow and its audit row use the saving/invocation client and user. | Closed by `5g-1a-y` and `5g-1b` | https://github.com/samqbush/adempiere2/pull/19 froze the corrected legacy answer at `439a35e65f31a2c3b72926c1bb21114934444590`; https://github.com/samqbush/adempiere2/pull/18 implemented the shared production correction at `e02c82ed5cae77ec9eb3067be291cb78332ffccc` without changing that oracle. Post-merge run 33932245213 retained the accepted attribution through the complete matrix. Owner @samqbush. |
| R15 | Phase 5e redirect and session-end transitions | PR 18 exposed transition failures in the inherited Phase 5e router: a theme image was refused during `REDIRECT_PENDING`, routed logout could strand the browser when AU and page responses raced for navigation ownership, and run 33889287297 proved the same allowlisted theme image could still be refused after the redirect flag cleared while affinity remained `PENDING_ROTATION`. | Closed by PR 20 and `5g-1b` | PR 20, https://github.com/samqbush/adempiere2/pull/20, closed the first two measured defects at merge commit `ccffe15ff065f391417a1b91419f93aa9634d28a`, and `5g-1b` reused the same contract-pinned `transitionSafeAsset` predicate during the remaining awaiting-context-root interval while writes, ordinary assets, generated ZK content, and all other requests stayed refused. Run 33893941634 completed both 12-step captures with no `progress2.gif` refusal and passed the no-legacy-fallback control; merged run 33932245213 retained the behavior. Owner @samqbush. |
| R16 | Routed modern `AD_Session` closure | Run 33893941634 destroyed all four modern servlet sessions and returned every runtime cache to zero, but left their four database `AD_Session` rows live (`Processed='N'`). `AdempiereWebUI.logout()` cleared the authenticated context before `SessionManager.clearSession()` read `#AD_Session_ID`, so the database logout path silently did nothing. | Closed by `5g-1b` | `AdempiereWebUI.logout()` now closes `AD_Session` before clearing the authenticated context, and the database-neutral gate pins that ordering. Run 33904468993 passed the complete lifecycle row with post-logout live GardenWorld sessions `0`, baseline `0`, both servlet sessions destroyed, and all modern runtime caches at zero; PR 18 merged the correction at `e02c82ed5cae77ec9eb3067be291cb78332ffccc`. Owner @samqbush. |
| R17 | Concurrent routed logout after modern-session destruction | Final-head run 33908565898 passed both captures, frozen comparison, and five H6 controls, then stranded the second-editor page on Tomcat 10's unbootstrapped-session 403. One logout request had already destroyed the modern session while a concurrent request still carried the routed modern cookie. | Closed by `5g-1b`; post-merge boundary regression tracked as R18 | The public router carries its reserved Tomcat 9 session binding on every internal routed request, not only bootstrap. Exact-head runtime run 33913861562 passed both captures and all six H6 rows, including the formerly failing session-cleanup driver, and PR 18 merged at `e02c82ed5cae77ec9eb3067be291cb78332ffccc`. Post-merge run 33922390350 then proved that binding alone was too broad a recovery signal; R18 owns that separate fail-closed regression. Owner @samqbush. |
| R18 | Backend restart misclassified as routed logout | Post-merge regression run 33922390350 failed only `phase5eCohortRoutingSmoke`. Its `missing-affinity` row restarted Tomcat 10, deliberately destroying the modern session while preserving Tomcat 9 affinity. The R17 branch treated the surviving router binding alone as proof of logout, emitted END, and returned to a successful public page instead of the required explicit failure. | Closed by PR 21 and post-merge run 33932245213 | The modern filter records the exact modern session identifier for 30 seconds when it destroys a session already marked logged out. A later no-session request repeats END only when it is loopback, carries the router-authored Tomcat 9 binding, and requests that exact recently ended modern identifier. Unknown identifiers - including every identifier lost by a backend restart, because the in-memory record restarts with Tomcat 10 - remain forbidden. PR 21 merged at `0e5b42c18522eb6e3925d9a367ef8c348223e994`; run 33932245213 passed the current Phase 5g-1b smoke and all eight historical regression lanes. Owner @samqbush. |
...
`5g-1a-y`, the R14 workflow-attribution oracle amendment, is **complete**. It
merged through https://github.com/samqbush/adempiere2/pull/19 at
`439a35e65f31a2c3b72926c1bb21114934444590` and adds no production runtime code.
...
disposable detached worktree, materializes the corrected runtime only under
`build/phase5g1ay/`, restores the ordinary installed runtime byte-for-byte
before scoring, and attaches an independent digest-backed Gradle restoration
finalizer for failures. Both Lane 1 jobs use full-history checkout so the
validator can distinguish the oracle branch from descendants of accepted merge
`439a35e65f31a2c3b72926c1bb21114934444590`; they still ensure the exact pinned
source object, and the materializer repeats that assurance before validation.
Before acceptance the exact oracle-only scope is enforced; on descendants,
`contracts/legacy-web-write-v1/` is immutable while separately reviewed
...
`5g-1b`, modern Business Partner CRUD parity, is **complete and accepted**. It
is the increment that first observed a modern business write: the modern ZK CE
...
**Mutation proof.** Twenty-four reviewed mutations are scored, and only behaviour is
...
| 5g-1b / PR 18 (**complete**, merged at `e02c82ed5`; R18 follow-up PR 21 merged at `0e5b42c18`) | Modern Business Partner CRUD parity, scored **only** through the public routed `/webui` origin, including the legacy two-user concurrency answer 5g-1a froze, the duplicate-submit answer 5g-1a-x froze, and the accepted workflow-attribution answer from 5g-1a-y. Adds the shared selector-dialect flow, routed capture lane, per-restore ambient census, `AD_Session` lifecycle model, H6 write-traffic matrix, fail-closed evidence validator, and the reviewed production saving-context correction. Exact-head run 33913861562 completed both modern captures and all six H6 rows; post-merge run 33932245213 passed the current smoke and every historical database-backed lane after the R18 boundary correction. `contracts/legacy-web-write-v1/` remains read-only; there is no modern contract tree or after-the-fact divergence list | Yes | **5g-1a**, as amended by **5g-1a-x** and **5g-1a-y**; accepted |
| 5g-1c | Sales Order draft -> Complete: document status, document number, reservations and tax. **No accounting** | Yes | **`5g-1c-oracle` named, not captured/reviewed/merged - blocking** |
