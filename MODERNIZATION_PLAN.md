...
| R18 | Backend restart misclassified as routed logout | Post-merge regression run 33922390350 failed only `phase5eCohortRoutingSmoke`. Its `missing-affinity` row restarted Tomcat 10, deliberately destroying the modern session while preserving Tomcat 9 affinity. The R17 branch treated the surviving router binding alone as proof of logout, emitted END, and returned to a successful public page instead of the required explicit failure. | Correction implemented; runtime acceptance pending | The modern filter now records the exact modern session identifier for 30 seconds when it destroys a session already marked logged out. A later no-session request repeats END only when it is loopback, carries the router-authored Tomcat 9 binding, and requests that exact recently ended modern identifier. Unknown identifiers - including every identifier lost by a backend restart, because the in-memory record restarts with Tomcat 10 - remain forbidden. Focused tests prove logout-race recovery, loopback missing-affinity refusal, and non-loopback refusal. The post-merge matrix remains stop-the-line until CI proves the corrected Phase 5e and current Phase 5g-1b lanes green. Owner @samqbush. |
