...
| `./gradlew firstModernDemoFinalVerification --dependency-verification=strict` | Database-neutral **current head of the phase-gate chain**: chains the accepted Phase 5g-1b gate and validates the first modern demo's exact runtime pins, loopback-only published surface, image digests, artifact allowlist, secret exclusions, lifecycle commands, and marker-owned reset boundary. It does not build images or touch a database. | `gradle/phase5/first-modern-demo.gradle`, `demo/first-modern-business/contract.properties`, `scripts/demo/validate-first-modern-demo-contract.py`. |
