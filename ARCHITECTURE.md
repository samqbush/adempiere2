...
> `f362c1f401c7c539a5e722f58abfb68d98d2b990` (2026-09-05).
>
...
| `./gradlew phase5cFinalVerification --dependency-verification=strict` | Database-neutral Phase 5c gate: deterministic transformer fixtures and report-only legacy scan, additive installed/release overlay, Phase 4 API preservation, binding ADR, and artifact rollback. Phase 5d removed its 503 marker smoke; the historical marker evidence is retained in `docs/modernization/phase-5c-evidence.md`. | `gradle/phase5/beachhead.gradle`, `zkwebui/build.gradle`, `scripts/phase5/`, `docs/modernization/phase-5c-evidence.md`. |
| `./gradlew phase5dFinalVerification --dependency-verification=strict` | Database-neutral Phase 5d gate: verifies the functional ZK CE 10 `/webui-modern` WAR (required login/session/menu/window classes and resources, no marker, no ZK 3.6, no `zkex`/`zkmax`/`org.zkforge`, no `javax`/`jakarta` servlet API, no stale signatures, no source or test classes, no undeclared multi-release entries, no duplicate classes), the current ZK source and namespace inventories, the frozen legacy artifact and its source crossing, the installed/release overlay, artifact rollback, Phase 4 preservation, and the still-valid Phase 5b/5c assertions. It does **not** chain `phase5cFinalVerification`. | `gradle/phase5/zk-functional-slice.gradle`, `gradle/phase5/zk-compile-closure.gradle`, `gradle/phase5/modern-web-shared-runtime.tsv`, `gradle/phase5/modern-web-assets.tsv`, `zkwebui/build.gradle`. |
| `./gradlew phase5dModernWebSmoke -Pphase3DbSystemPassword='<password>' --dependency-verification=strict` | Database-backed Phase 5d gate and web UI Testability Milestone: boots the modern ZK CE 10 slice beside the unchanged Phase 4 CXF WAR in one loopback Tomcat 10.1.59 JVM, drives ordinary GardenAdmin login, role selection, desktop/menu and the read-only "Error Message" window twice with a marker-guarded fixture reset between captures, self-diffs the two captures, compares eleven semantic facts and the zero-write database effect with the frozen legacy baseline at matching capture ordinals, then replays the complete Phase 4 SOAP corpus in a third session while a modern ZK session is authenticated and records listener, heap, classloader and `ADEMPIERE_HOME` evidence with the Tomcat 9 lane running concurrently. | `gradle/phase5/zk-functional-slice.gradle`, `zkwebui/src/modernBrowserTest/`, `scripts/phase5/start-modern-web-lane.sh`, `scripts/phase5/capture-modern-coexistence.sh`, `contracts/legacy-web-browser-v1/`. |
...
| `./gradlew phase5g0FinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-0 discovery gate: generate-then-validate of the three reviewed Phase 5g inventories with an exact row and preamble comparison and a closed contract directory listing, chaining `phase5fFinalVerification`. | `gradle/phase5/phase5g-inventories.gradle`, `scripts/phase5/generate-phase5g-inventories.py`, `scripts/phase5/validate-phase5g-inventories.py`. |
| `./gradlew phase5g1aFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1a write-oracle gate: verifies the `contracts/legacy-web-write-v1/` manifest and its required-file content floor, re-derives the table-scoped callout and registered-validator attribution to prove `C_BPartner` carries neither, scores the write-capture normalizer in both directions against the committed raw fixture, and chains `phase5g0FinalVerification`. Ships no modern runtime code and proves no write parity. | `gradle/phase5/write-oracle.gradle`, `contracts/legacy-web-write-v1/`, `scripts/phase5/generate-phase5g-write-attribution.py`, `scripts/phase5/validate-phase5g-write-attribution.py`, `scripts/phase5/verify-write-normalizer-mutation-proof.py`. |
| `./gradlew phase5g1ayFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1a-y workflow-attribution oracle amendment: chains `phase5g1aFinalVerification`; verifies the dedicated contract manifest, accepted merge, exact corrected-legacy patch, capture-only materialization, ordinary-runtime restoration, saving-context attribution, and 62 fail-closed mutations. It makes no production runtime or modern parity claim. | `gradle/phase5/write-oracle.gradle`, `contracts/phase5g1ay-workflow-attribution-v1/`, `scripts/phase5/validate-phase5g1ay-oracle.py`, `scripts/phase5/validate-phase5g1ay-provenance.py`, `scripts/phase5/verify-phase5g1ay-mutation-proof.py`. |
| `./gradlew phase5g1aLegacyWriteOracleSmoke -Pphase3DbSystemPassword='<password>' -Pphase5g1ayMode=corrected-legacy-workflow-attribution --dependency-verification=strict` | Database-backed Phase 5g-1a-y capture/acceptance mode. After the ordinary Phase 3 product is built, Gradle snapshots both installed jars, the materializer ensures the exact pinned source object exists before validation, applies the pinned patch only in a detached worktree, compiles the three corrected workflow classes, removes the exact contract-pinned stale code-signature entries, and materializes a corrected `Adempiere.jar` only under `build/phase5g1ay/`. The two captures must each contain the keyed process, activity, and event-audit rows with saving-context attribution before scoring. The external guard activates the jar for the lane and restores both ordinary jars before scoring; a Gradle finalizer independently restores and digest-verifies them even when the smoke fails. Provenance requires `repository_head == git rev-parse HEAD`, the exact removed-signature list, and exactly the corrected jar plus the three top-level class outputs. Omitting the property (or matching environment variable) preserves the existing 5g-1a/5g-1a-x legacy behavior. **Candidate run 33785079015 is green; separate freeze-off run 33788686426 accepted the committed facts with A/B self-diff `pass` and zero scoring problems.** | `scripts/phase5/guard-phase5g1ay-ordinary-runtime.sh`, `scripts/phase5/materialize-phase5g1ay-corrected-runtime.sh`, `scripts/phase5/run-write-oracle-smoke.sh`, `contracts/phase5g1ay-workflow-attribution-v1/`, `docs/modernization/phase-5g-1a-y-evidence.md`. |
| `./gradlew phase5g1bFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1b write-parity gate: chains `phase5g1ayFinalVerification`, proves the runtime evidence validator rejects every declared defect class, asserts the lane invariants that forbid a parity increment from producing its own answer or bypassing the public routed `/webui` origin, and verifies the production process/activity/event-audit construction chain inherits the saving invocation context while preserving the old workflow APIs. | `gradle/phase5/write-parity.gradle`, `scripts/phase5/validate-phase5g1b-runtime-evidence.py`, `scripts/phase5/verify-phase5g1b-runtime-evidence-validator.py`, `scripts/phase5/run-write-parity-smoke.sh`, `base/src/org/compiere/wf/DocWorkflowManager.java`, `base/src/org/compiere/wf/MWorkflow.java`, `base/src/org/compiere/wf/MWFProcess.java`. |
| `./gradlew phase5g1bModernWriteParitySmoke -Pphase3DbSystemPassword='<password>' -Pphase5g1ayMode=corrected-legacy-workflow-attribution --dependency-verification=strict` | Database-backed Phase 5g-1b gate and **current-phase smoke**: re-proves the corrected legacy write oracle at PR HEAD, captures the modern Business Partner flow twice through the public routed `/webui` origin from independent seed restores, scores both against the frozen contract, runs the H6 write-traffic matrix, and validates runtime evidence fail-closed. PR 21 merged the final R18 boundary correction at `0e5b42c18522eb6e3925d9a367ef8c348223e994`; post-merge run 33932245213 passed this smoke and all eight historical database-backed lanes. | `gradle/phase5/write-parity.gradle`, `scripts/phase5/run-write-parity-lane.sh`, `scripts/phase5/routed-ambient-census.sh`, `zkwebui/src/writeParitySupport/`, `docs/modernization/phase-5g-1b-evidence.md`. |
| `./gradlew firstModernDemoFinalVerification --dependency-verification=strict` | Database-neutral **current head of the phase-gate chain**: chains the accepted Phase 5g-1b gate and validates the first modern demo's exact runtime pins, loopback-only published surface, image digests, artifact allowlist, secret exclusions, lifecycle commands, marker-owned reset boundary, and public browser save-then-read-back contract. It does not build images or touch a database. The corresponding downloaded-artifact lifecycle passed in run [33994400756](https://github.com/samqbush/adempiere2/actions/runs/33994400756) at `f362c1f40`. | `gradle/phase5/first-modern-demo.gradle`, `demo/first-modern-business/contract.properties`, `scripts/demo/validate-first-modern-demo-contract.py`, `zkwebui/src/writeParityTest/java/org/adempiere/webui/phase5g/FirstModernDemoPublicOriginTest.java`. |
| `./gradlew prepareFirstModernDemoContext --dependency-verification=strict` | Heavy CI-only distribution task. Builds/stages the accepted unconfigured release product and two-Tomcat routing overlay, rejects configured files and runtime residue, and adds the seed plus reviewed image runtime scripts. Containers consume this output; they do not run Gradle or Ant. | `gradle/phase5/first-modern-demo.gradle`, `.github/workflows/first-modern-business-demo.yml`, `demo/first-modern-business/runtime/`. |
...
**`zkwebui` is the exception to that rule.** From Phase 5d the Ant reactor no
longer *compiles* the migrated ZK trees. The legacy `webui.war` and the five ZK
package jars are materialized from the frozen Phase 5b source commit in an
isolated worktree - `zkwebui/build.xml` imports
`utils_dev/phase5d-frozen-zk.xml` and its `war` target depends on
`phase5d.materialize-frozen-zk`, copying the frozen archive rather than building
one - while Gradle owns the modern `/webui-modern` WAR. Editing `zkwebui`
sources therefore does **not** change the deployed legacy artifact; see
`gradle/phase5/legacy-web-artifact.gradle` and
`gradle/phase5/zk-functional-slice.gradle`.
...
| First modern business demo containers | GitHub Actions-built `linux/amd64` OCI application and PostgreSQL 14.6 seed images distributed as a downloadable Compose bundle | The application image contains the CI-built installed product and both routed Tomcat runtimes; the database remains internal, Tomcat 10 remains unpublished, and only `127.0.0.1:8888` is exposed through a separate ingress network. Runtime secrets are generated after download. The complete clean-directory lifecycle passed in run [33994400756](https://github.com/samqbush/adempiere2/actions/runs/33994400756) (`demo/first-modern-business/`, `.github/workflows/first-modern-business-demo.yml`, `docs/modernization/first-modern-business-demo.md`). |
