...
| `./gradlew phase5g1aFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1a write-oracle gate and **current head of the phase-gate chain**: verifies the `contracts/legacy-web-write-v1/` manifest and its required-file content floor, re-derives the table-scoped callout and registered-validator attribution to prove `C_BPartner` carries neither, scores the write-capture normalizer in both directions against the committed raw fixture, and chains `phase5g0FinalVerification`. Ships no modern runtime code and proves no write parity. | `gradle/phase5/write-oracle.gradle`, `contracts/legacy-web-write-v1/`, `scripts/phase5/generate-phase5g-write-attribution.py`, `scripts/phase5/validate-phase5g-write-attribution.py`, `scripts/phase5/verify-write-normalizer-mutation-proof.py`. |
