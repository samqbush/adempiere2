...
| `./gradlew phase5g1bFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1b write-parity gate and **current head of the phase-gate chain**: proves the runtime evidence validator rejects all 29 declared defect classes, asserts the lane invariants that keep a parity increment from producing the answer it is scored against (no `--freeze` to the scorer, no modern contract tree, the public routed `/webui` origin and never the loopback modern one, and a per-session serving-runtime assertion), and chains `phase5g1aFinalVerification`. | `gradle/phase5/write-parity.gradle`, `scripts/phase5/validate-phase5g1b-runtime-evidence.py`, `scripts/phase5/verify-phase5g1b-runtime-evidence-validator.py`, `scripts/phase5/run-write-parity-smoke.sh`. |
| `./gradlew phase5g1bModernWriteParitySmoke -Pphase3DbSystemPassword='<password>' --dependency-verification=strict` | Database-backed Phase 5g-1b gate and **current-phase smoke**: re-proves the legacy write oracle at PR HEAD, then captures the modern Business Partner write flow twice through the public routed `/webui` origin from independently restored databases, scores both against the frozen legacy contract with freezing off, runs the H6 write-traffic matrix with a full seed restore per destructive row, and validates the runtime evidence fail-closed. Executed repeatedly and **never green**; latest run [33696036502](https://github.com/samqbush/adempiere2/actions/runs/33696036502), still failing on residual risk R14. | `gradle/phase5/write-parity.gradle`, `scripts/phase5/run-write-parity-lane.sh`, `scripts/phase5/routed-ambient-census.sh`, `zkwebui/src/writeParitySupport/`. |
| `./gradlew phase5g1aFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1a write-oracle gate, now chained beneath `phase5g1bFinalVerification`: verifies the `contracts/legacy-web-write-v1/` manifest and its required-file content floor, re-derives the table-scoped callout and registered-validator attribution to prove `C_BPartner` carries neither, scores the write-capture normalizer in both directions against the committed raw fixture, and chains `phase5g0FinalVerification`. Ships no modern runtime code and proves no write parity. | `gradle/phase5/write-oracle.gradle`, `contracts/legacy-web-write-v1/`, `scripts/phase5/generate-phase5g-write-attribution.py`, `scripts/phase5/validate-phase5g-write-attribution.py`, `scripts/phase5/verify-write-normalizer-mutation-proof.py`. |
