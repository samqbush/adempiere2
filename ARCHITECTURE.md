...
| `./gradlew phase5g1ayFinalVerification --dependency-verification=strict` | Database-neutral Phase 5g-1a-y workflow-attribution oracle amendment and **current head of the phase-gate chain**: chains `phase5g1aFinalVerification`; always re-runs against dynamic Git state; verifies the dedicated contract manifest, source commit, accepted PR 19 merge commit, exact patch SHA-256, hunk headers, executable changed-line set and three-path allowlist; before acceptance it enforces the pinned-source-to-`HEAD` oracle-only scope and protected roots, while descendants of the accepted merge may change or rename production code but cannot change or rename the frozen legacy write oracle in committed or working-tree state; proves the patch carries the explicit invocation `Properties` through backward-compatible overloads without side effects or transaction changes; validates fetch-before-validation, exact fail-closed owned-worktree cleanup, exact signed-jar signature removal, the independent runtime guard/finalizer, pre-score workflow-row attribution guards, and provenance requiring the current repository `HEAD` plus the exact corrected jar/three-class inventory; and runs 62 fail-closed mutations. It makes no runtime attribution or modern parity claim. | `gradle/phase5/write-oracle.gradle`, `contracts/phase5g1ay-workflow-attribution-v1/`, `scripts/phase5/validate-phase5g1ay-oracle.py`, `scripts/phase5/validate-phase5g1ay-provenance.py`, `scripts/phase5/verify-phase5g1ay-mutation-proof.py`. |
