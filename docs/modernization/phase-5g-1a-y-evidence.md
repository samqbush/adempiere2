...
**Status: corrected legacy oracle accepted and merged.** Candidate run
...
|---|---|
| Exact source and accepted merge | `capture-contract.tsv` pins source `60af453aab0e6537a6189241366c857c239c959f` and accepted merge `439a35e65f31a2c3b72926c1bb21114934444590`; both Lane 1 jobs use full history for the ancestry decision and ensure the exact source object exists, and the materializer repeats the source-object check before validation |
...
| Oracle isolation and descendant protection | Before acceptance, the neutral validator enforces the exact oracle-branch path allowlist and protected production/artifact roots. After accepted merge `439a35e65f31a2c3b72926c1bb21114934444590` is an ancestor, later branches may change production code but both committed and working-tree changes to `contracts/legacy-web-write-v1/` fail closed |
...
- pre-acceptance production-source and ordinary-artifact immutability from the
  pinned source commit through `HEAD`, plus exact oracle-branch scope; or, on
  descendants of the accepted merge, immutability of the frozen legacy write
  oracle in committed and working-tree state;
...
- 62 mutation cases covering cached-context regressions, altered overload
  delegation, side effects, fetch bypass, working-tree/committed-scope
  weakening, accepted-merge pinning, pre-acceptance rename rejection,
  downstream frozen-oracle changes and renames,
  unsafe worktree cleanup, activation/restoration bypass,
...
| Merge to `develop` | **PR 19 merged as `439a35e65f31a2c3b72926c1bb21114934444590`** |

PR 18 may consume this accepted oracle but must not change its frozen facts.
