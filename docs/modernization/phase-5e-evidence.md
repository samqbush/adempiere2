...
| During `REDIRECT_PENDING` and the subsequent `PENDING_ROTATION` wait for context root, only `GET`/`HEAD` `.gif`/`.png` files under `/theme/default/images/` may pass through the legacy chain; the barrier and affinity stay unchanged, and `/zkau/view/`, ZK resources, pages, AU requests, unknown routes, path parameters, and write methods remain refused | `PublicRouteClassifierTest`, `RoutingCoreTest`, `CohortRoutingFilterTest`, `verifyPhase5eCohortContracts` |
| A routed session end has exactly one cleanup owner and one navigation owner per transport. Page END uses an HTTP redirect, AU END uses the ZK `redirect` response, concurrent cross-transport responses target the same context root, same-transport duplicates cannot redirect again, committed responses do not consume ownership, and the next request is undecided | `RoutingCoreTest`, `CohortRoutingFilterTest` |
...
| Theme image overlaps the role-selection redirect transition | At least one cache-bypassed `GET /theme/default/images/zk/progress2.gif?r15=...` is recorded with `transition-safe-asset`, the closed safe set remains readable both while the marked barrier is live and while the affinity awaits context root, and the session reaches the modern desktop. Other post-barrier requests retain the fail-closed `awaiting-context-root` response |
...
| Transition states refused immutable theme images | `PublicRouteClassifier`, `RoutingCore`, `CohortRoutingFilter` | PR 18 run 33696036502 measured a 503 for `/theme/default/images/zk/progress2.gif` while `REDIRECT_PENDING` was set; run 33889287297 measured the same refusal after that flag cleared while affinity still awaited context root. Transition safety is a separate closed predicate, not an alias for `STATIC_ASSET`: only `GET`/`HEAD` `.gif`/`.png` files under `/theme/default/images/` pass through during either interval, without releasing the barrier or touching affinity, cookies, rotation, bootstrap, or tickets. Run 33893941634 completed both 12-step captures without the refusal and passed the corrected no-legacy-fallback control. |
| Every END response received an ordinary HTTP redirect and every concurrent request repeated cleanup/navigation | `ModernSessionAffinity`, `RoutingLifecycle`, `CohortRoutingFilter` | PR 18 run 33704883870 measured concurrent AU/page END responses followed by no rendered login form. A 302 returned to ZK AU/XHR does not own top-level navigation. The first R15 implementation changed AU to the exact ZK redirect command, but run 33810259720 proved that was insufficient: the 36-byte command was delivered after ZK had already started `/webui/index.zul`, while that top-level request lost the global navigation claim and received 204. Cleanup now has one atomic owner, navigation has one owner per transport, AU and page responses both name the same context root, same-transport duplicates return 204, and committed responses leave that transport available. The browser evidence records sanitized AU status/content headers, exact-command recognition, and URLs before/after logout. |
