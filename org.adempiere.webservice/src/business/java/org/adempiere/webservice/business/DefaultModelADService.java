...
/**
 * Transport-neutral business implementation of the eight request-scoped
 * {@code ModelADService} operations. The behavior is the mechanical extraction
 * of {@code com._3e.ADInterface.ModelADServiceImpl}; only the fault type
 * changed from the transport framework fault to {@link ServiceFault}.
 *
 * @author kolec
 */
public class DefaultModelADService implements ModelADServiceBusiness {

	/**
	 * The legacy logger name is preserved verbatim so extracted business
	 * logging keeps emitting under the historical category.
	 */
	private static CLogger	log =
	CLogger.getCLogger("com._3e.ADInterface.ModelADServiceImpl");
	...
	public DefaultModelADService()
	{
	this(newConnectedCompiereService());
	}

	public DefaultModelADService(CompiereService compiereService)
	{
	m_cs = compiereService;
	}

	private static CompiereService newConnectedCompiereService()
	{
	CompiereService compiereService = new CompiereService();
	compiereService.connect();
	return compiereService;
	}
	...
	ModelSetDocActionRequestDocument req) throws ServiceFault {
	...
	private String validateParameter(String parameterName, String string) throws ServiceFault {
	...
	if (para == null)
	throw new ServiceFault("validateParameter",
	"Web service type "
	...
	private int validateParameter(String parameterName, int i) throws ServiceFault {
	...
	private ModelCRUD.Action.Enum validateParameter(String parameterName, ModelCRUD.Action.Enum action, Table table) throws ServiceFault {
	String string = null;
	if (action == null)
	...
	private ModelCRUD.RetriveResultAs.Enum validateParameter(String parameterName, ModelCRUD.RetriveResultAs.Enum  retriveResultAs, Table table) throws ServiceFault {
	String string = null;
	...
	private String modelLogin(ADLoginRequest r, String webService, String method, String serviceType) throws ServiceFault {
	...
	private String authenticate(String webServiceValue, String methodValue, String serviceTypeValue) throws ServiceFault {
	...
	{
	throw new ServiceFault("authenticate", e.getClass().toString() + " " + e.getMessage() + " sql=" + sql, null, e.getCause());
	}
	finally
	{
	DB.close(rs, pstmt);
	...
	@Override
	public RunProcessResponseDocument runProcess(ModelRunProcessRequestDocument modelRunProcessRequestDocument) throws ServiceFault {
	...
	DB.close(rs, pstmt);
	rs = null; pstmt = null;
	throw new ServiceFault("getList", e.getClass().toString() + " " + e.getMessage() + " sql=" + sql, null, e.getCause());
	}
	...
	if (rt == null)
	throw new ServiceFault("getList",
	"Web service type "
	...
	private void validateCRUD(ModelCRUD modelCRUD) throws ServiceFault {
	...
	DataField field, int idxcol) throws ServiceFault {
	// Evaluate the type of the column and assign a proper variable
	Object value = toColumnValue(
	poinfo.getColumnClass(idxcol), field, m_webservicetype.getValue());
	if (!po.set_ValueOfColumnReturningBoolean(field.getColumn(), value))
	throw new ServiceFault("setValueAccordingToClass",
	"Cannot set value of column "
	+ field.getColumn(),
	null);
	}

	/**
	 * Converts one {@link DataField} value to the persistence value the column
	 * class requires. Extracted verbatim from the legacy
	 * {@code setValueAccordingToClass} body so the conversion and its exact
	 * fault messages can be exercised without a database.
	 *
	 * @param columnClass the {@code POInfo} column class
	 * @param field the requested field
	 * @param webServiceTypeValue the {@code WS_WebServiceType.Value} used in
	 *        fault messages
	 * @return the converted value, possibly {@code null}
	 * @throws ServiceFault when the requested value is not convertible
	 */
	static Object toColumnValue(Class<?> columnClass, DataField field,
	String webServiceTypeValue) throws ServiceFault {
	Object value = null;
	...
	throw new ServiceFault("setValueAccordingToClass",
	"Web service type "
	+ webServiceTypeValue + ": input column "
	...
	throw new ServiceFault("setValueAccordingToClass", e.getClass().toString() + " " + e.getMessage() + " for " + field.getColumn(), null, e.getCause());
	}
	...
	@Override
	public WindowTabDataDocument readData(ModelCRUDRequestDocument modelCRUDRequestDocument) throws ServiceFault {
	...
	MTable table = MTable.get(ctx, tableName);
	if (table == null)
	throw new ServiceFault("readData",
	"Web service type "
	...
	@Override
	public WindowTabDataDocument queryData(ModelCRUDRequestDocument modelCRUDRequestDocument) throws ServiceFault {
	WindowTabDataDocument windowTabDataDocument = WindowTabDataDocument.Factory.newInstance();
	WindowTabData windowTabData = windowTabDataDocument.addNewWindowTabData();
	ModelCRUD modelCRUD = modelCRUDRequestDocument.getModelCRUDRequest().getModelCRUD();
	String serviceType = modelCRUD.getServiceType();
	...
	MTable table = MTable.get(ctx, tableName);
	if (table == null)
	throw new ServiceFault("queryData",
	"Web service type "
	+ m_webservicetype.getValue() + ": table "
	+ tableName + " not found",
	null);
	...
	"Web service type "
	+ m_webservicetype.getValue() + ": input column "
	+ field.getColumn() + " not allowed", null);
	}
	}
