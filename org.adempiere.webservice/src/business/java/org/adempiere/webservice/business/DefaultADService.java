...
import com._3e.ADInterface.ADLookup;
import com._3e.ADInterface.CompiereService;
import com._3e.ADInterface.Process;
import com._3e.ADInterface.WWindowStatus;
 ...
 *
 */
public class DefaultADService implements ADServiceBusiness {

	private static CLogger	log = CLogger.getCLogger("com._3e.ADInterface.ADServiceImpl");
    ...
    public WindowDocument getADWindow(int WindowNo, int AD_Window_ID, int AD_Menu_ID) throws ServiceFault   {
...
//	method to convert Document to String
    ...
    void authenticate(String webServiceName, String method) throws ServiceFault {
	if (!m_cs.isLoggedIn())
	...
	private ServiceFault authenticationRequired() {
	Exception cause = new Exception("You need to login");
    ...
    public WindowTabDataDocument getWindowTabData(WindowTabDataReqDocument reqd) throws ServiceFault {
	...
	ws.getCurrentTab().query(ws.getWindow().isTransaction()); //!!!!!!!!!!
	ws.getCurrentTab().navigate(0);
	ws.getCurrentTab().setSingleRow(true);
	}

	if (ws.getCurrentTab().getTabNo() != req.getPrevTabNo())
	{
	ws.getCurrentTab().removeDataStatusListener(ws.ads);
	ws.setCurrentTab(ws.getWindow().getTab(req.getPrevTabNo()));
	ws.getCurrentTab().query(ws.getWindow().isTransaction()); // false
	ws.getCurrentTab().navigate(0);
	ws.updateRecIDMap();
	}
	/*
	if (ws.getCurrentTab().getCurrentRow() != req.getPrevRecNo())
	{
	if (req.getPrevRecNo() >=0)
	ws.getCurrentTab().navigate( req.getPrevRecNo() );
	}
	*/
	int prevRecNo = ws.getRowNoFromRecordID( req.getPrevRecNo()); // we assume that it RecordID
	if (ws.getCurrentTab().getCurrentRow() != prevRecNo)
	{
	if (prevRecNo >=0)
	ws.getCurrentTab().navigate( prevRecNo );
	}
	...
	System.out.println(ws2.getCurrentTab().getTableName());
	GridField field = ws2.getCurrentTab().getField(req.getFromZoomColumnName());
	ws2 = null;
	...
	ws.getCurrentTab().setQuery(zoomQuery);
	//ws.getCurrentTab().query(ws.getWindow().isTransaction());

	// ADEMPIERE/COMPIERE
	ws.getCurrentTab().query(false, 0, 0); // adempiere
	//ws.getCurrentTab().query(false, 0); // compiere

	} else
	{
	DataRow findDR = req.getFindCriteria();
	MQuery currentQuery = ws.getCurrentTab().getQuery();
	MQuery newQuery = createQuery( ws.getCurrentTab().getTableName(), findDR );
	...
	ws.getCurrentTab().setQuery(newQuery);
	//ws.getCurrentTab().query(ws.getWindow().isTransaction());
	...
	rc = req.getRowCount(); else rc = ws.getCurrentTab().getRowCount();
	int initRowNo = 0;
	...
	wd.setTotalRows( ws.getCurrentTab().getRowCount() ); // ok
	...
	{
	ws.getCurrentTab().navigate(lineNo);

	int recID = ws.getCurrentTab().getRecord_ID();
	...
	if ( lastRow <= ws.getCurrentTab().getRowCount() ) {  //last row
	ws.updateRecIDMap();
	}
	}
    ...
    public WindowTabDataDocument getDataRow(int WindowNo, int TabNo, int RowNo )  throws ServiceFault  {
    ...
    public WindowTabDataDocument updateDataRow(int WindowNo, int TabNo, int RowNo, WindowTabDataDocument data )  throws ServiceFault  {
    ...
    private void fillDataRow( DataRow dr, WWindowStatus ws, boolean handleLookups, boolean onlyUpdated )   throws ServiceFault {
	authenticate(webServiceName, "fillDataRow");

	int noFields = ws.getCurrentTab().getFieldCount();
	...
	java.util.Date updated = (java.util.Date)ws.getCurrentTab().getValue("Updated");
	if (!updated.after(clientUpdated)) {
	DataField df = dr.addNewField();
	df.setColumn( recordIDfield );
	df.setDisp(false);
	df.setVal( Integer.toString( ws.getCurrentTab().getRecord_ID() ));
	return;
	}
	}
	...
	{
	GridField field = ws.getCurrentTab().getField(colNo);
	...
	Object data = ws.getCurrentTab().getValue( column );
	String info = null;
    ...
    public WindowTabDataDocument saveDataRow(int WindowNo, int TabNo, int RowNo, WindowTabDataDocument data ) throws ServiceFault {
	...
	//  Check Mandatory
	int size = ws.getCurrentTab().getFieldCount();
	for (int i = 0; i < size; i++)
	{
	GridField field = ws.getCurrentTab().getField(i);
	...
	wd.setSuccess( ws.getCurrentTab().dataSave(true) );
	;//ws.getCurrentTab().dataIgnore();
	//else
	...
	GridField GridField = ws.getCurrentTab().getField( cn  );
	...
	ws.getCurrentTab().setValue (GridField, null);
	...
	{
	ws.m_needSave = true;
	GridField GridField = ws.getCurrentTab().getField(columnName);
	if (GridField != null)
	{
	...
	ws.getCurrentTab().setValue(GridField, newValue);
	}
	}
	}
	*/
	...
	public WindowTabDataDocument addNewDataRow(int WindowNo, int TabNo ) throws ServiceFault	{
	...
	if (ws != null)
	{
	if (ws.getCurrentTab().dataNew(false))
	{
	//ws.getCurrentTab().setSingleRow(true);
	wd.setSuccess(true);
	DataRow dr = ds.addNewDataRow();
	fillDataRow( dr, ws, true, false );
	updateRecIDMap( ws );
	System.out.println("New Row no = "+ws.getCurrentTab().getCurrentRow());
	} else
	{
	ws.getCurrentTab().dataIgnore();
	wd.setSuccess(false);
	...
	public WindowTabDataDocument deleteDataRow(int WindowNo, int TabNo, int RowNo ) throws ServiceFault	{
	...
	public WindowTabDataDocument ignoreDataRow(int WindowNo, int TabNo, int RowNo ) throws ServiceFault	{
	...
	public WindowTabDataDocument refreshDataRow(int WindowNo, int TabNo, int RowNo ) throws ServiceFault	{
	...
	public WindowTabDataDocument getLookupSearchData(GetLookupSearchDataReqDocument req) throws ServiceFault
	...
	public WindowTabDataDocument getLookupData(int WindowNo, int TabNo, int RowNo, String columnName ) throws ServiceFault
	{
	...
	DataRow dr = ds.addNewDataRow();
	DataField df = dr.addNewField();
	GridField field = ws.getCurrentTab().getField(columnName);
	Lookup lookup = field.getLookup();
    ...
    public ADMenuDocument getADMenu(int AD_Role_ID) throws ServiceFault
    {
	...
	public ADLoginResponseDocument login( ADLoginRequestDocument req ) throws ServiceFault
	{
    ...
    public ProcessParamsDocument getProcessParams( GetProcessParamsDocument req ) throws ServiceFault
    {
	authenticate(webServiceName, "getProcessParams");
	return Process.getProcessParams(m_cs, req );
    }

    public RunProcessResponseDocument runProcess( RunProcessDocument req )  throws ServiceFault
    {
	authenticate(webServiceName, "runProcess");
	return Process.runProcess(m_cs, req);
    }

    public StandardResponseDocument saveLocation( LocationDocument req ) throws ServiceFault
    {
    ...
    public LocationDocument getLocation( LocationDocument req ) throws ServiceFault
    {
	...
	public DocActionDocument getDocAction(int WindowNo, int TabNo, int RowNo, String ColName ) throws ServiceFault
	{
	...
	Process.renderDocActionOptions(lvs, ws.getCurrentTab());
	}

	return ret;
	}

	public StandardResponseDocument setDocAction(int WindowNo, int TabNo, int RowNo, String ColName, String docAction ) throws ServiceFault
	{
	...
	if (ws != null)
	{
	ws.getCurrentTab().setValue("DocAction", docAction);
	boolean result = false;
	if (ws.getCurrentTab().needSave(true, false)) //slain - do not dispose of error, if not write musiales
	{
	if (! (result = ws.getCurrentTab().dataSave(true)))
	ws.getCurrentTab().dataIgnore();
	...
	ws.updateRecIDMap();
	/*
	int rc = ws.getCurrentTab().getRowCount();
	int initRowNo = 0;
	...
	{
	int recID = ws.getCurrentTab().getKeyID( lineNo );
